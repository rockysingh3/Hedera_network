...
class Executable {
  ...
  _shouldRetryExceptionally(error) {
    return error.status._code === _GrpcStatus.default.Unavailable._code || error.status._code === _GrpcStatus.default.ResourceExhausted._code || error.status._code === _GrpcStatus.default.Internal._code && RST_STREAM.test(error.message);
  }
  /**
  ...
  async execute(client, requestTimeout) {
    ...
    ;; attempt += 1) {
      ...
      _jsLogger.default.debug(`[${logId}] Node AccountID: ${node.accountId.toString()}, IP: ${node.address.toString()}`);
      ...
      if (!node.isHealthy()) {
        _jsLogger.default.debug(`[${logId}] node is not healthy, waiting ${node.getRemainingTime()}`);
      ...
      try {
        ...
        const error = _GrpcServiceError.default._fromResponse(
        /** @type {Error} */
        err);

        _jsLogger.default.debug(`[${logId}] received gRPC error ${JSON.stringify(error)}`);

        if (error instanceof _GrpcServiceError.default && this._shouldRetryExceptionally(error) && attempt <= maxAttempts) {
